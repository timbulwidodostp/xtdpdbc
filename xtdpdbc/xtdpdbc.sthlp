...
{pstd}
{cmd:xtdpdbc} implements the bias-corrected method-of-moments estimator of Breitung, Kripfganz, and Hayakawa (2022) for linear dynamic panel data models with unobserved group-specific effects,
...
see Theorem 1(i) in Breitung, Kripfganz, and Hayakawa (2022). When option {opt nocorrection} is specified, {cmd:vce(conventional)} is equivalent to {cmd:vce(unadjusted)}.
...
A warning message is displayed if this is not the case. In such a situation, as discussed by Breitung, Kripfganz, and Hayakawa (2022), alternative initial values should be used until a correct solution is found.
...
{it:Econometrics and Statistics} 24: 116-132.
